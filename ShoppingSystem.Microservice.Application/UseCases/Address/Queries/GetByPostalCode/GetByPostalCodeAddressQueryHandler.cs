...
public sealed class GetByPostalCodeAddressQueryHandler
    : IRequestHandler<GetByPostalCodeAddressQuery,
        Response<PagedResult<AddressResponseDto>>>
{
    private readonly IAddressService _addressService;
    private readonly IMapper _mapper;
    private readonly IValidator<GetByPostalCodeAddressQuery> _validator;

    public GetByPostalCodeAddressQueryHandler(
        IAddressService addressService,
        IMapper mapper,
        IValidator<GetByPostalCodeAddressQuery> validator)
    {
    ...
    public async Task<Response<PagedResult<AddressResponseDto>>> Handle(
        GetByPostalCodeAddressQuery request,
        CancellationToken ct)
    {
        ...
        var result = await _addressService.GetByPostalCodeAsync(
            mappedCriteria,
            request.PostalCode,
            ct);
        ...
        if (!result.Succeeded || result.Data is null)
        {
            ...
            {
                Succeeded = false,
                Message = result.Message ?? "No addresses found for this postal code.",
